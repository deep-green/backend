...
const socketAi2 = require('socket.io-client')('http://ec2-54-93-171-91.eu-central-1.compute.amazonaws.com:5000');
...
socketAi1.on('makeMove', (data) => {
    if(dg_interface.check_makeMove(data) === true) {
        setUserTokenIfNull(socketAi1.id, data.token);
        ...
        db_connector.ActiveGameData.findById(data.ID_game, 'socketRoom', (err, game)=> {
            ...
            socketAi1.to(game.socketRoom).emit('receive', dg_interface.emit_receive(data.FEN, data.ID_game, data.color));
...
socketAi2.on('makeMove', (data) => {
    if(dg_interface.check_makeMove(data) === true) {
        setUserTokenIfNull(socketAi2.id, data.token);
        ...
        db_connector.ActiveGameData.findById(data.ID_game, 'socketRoom', (err, game)=> {
            ...
            //send 'receive' to other player and all viewers watching the game
            socketAi2.to(game.socketRoom).emit('receive', dg_interface.emit_receive(data.FEN, data.ID_game, data.color));
...
io.on('connection', (client) => {
    ...
    client.on('makeMove', (data) => {
        if(dg_interface.check_makeMove(data) === true) {
            console.log('received "makeMove" from ' + data.token);
    ...
    client.on('newGame', (data) => {
        if(dg_interface.check_newGame(data) === true) {
            ...
            console.log('got newGame by ' + data.token + ' data: ' + console.log(util.inspect(data, {showHidden: false, depth: null})));
            setUserTokenIfNull(client.id, data.token);
            ...
            if((data.ID_enemy !== 'ki_1') && (data.ID_enemy !== 'ki_2')) {
                ...
                if(enemySocket) {
                    ...
                    client.emit('receive', dg_interface.emit_receive(data.FEN, newGame.ID_game, data.color));
