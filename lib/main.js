...
io.on('connection', (client) => {
    ...
    client.on('disconnect', ()=> {
        ...
        let filter = {
            whiteID: activeConnections[client.id]
        };

        db_connector.ActiveGameData.find(filter,'socketRoom', (resultSet)=> {
        ...
        if(dg_interface.check_viewGame(data) === true) {
            ...
            db_connector.ActiveGameData.findById(data.ID_game, 'viewers socketRoom', (err, game)=> {
                ...
                if(err) {
                    ...
                    if(activeConnections[client.id].token != null) {
                        game.viewers.push(activeConnections[client.id].token);
