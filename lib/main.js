...
//helper function - verifying and setting of User Token since no registration is available
...
io.on('connection', (client) => {
    ...
    if(alreadyConnected !== undefined) {
        ...
        console.log('new user connected. Socket-ID = ' + client.id);
    };
    ...
    client.on('viewGame', (data) => {
        if(dg_interface.check_viewGame(data) == true) {
            db_connector.ActiveGameData.findById(data.ID_game, 'viewers socketRoom', (err, game)=> {
                ...
                db_connector.ActiveGameData.findOneAndUpdate(data.ID_game, )
