...
io.on('connection', (client) => {
    ...
    client.on('disconnect', ()=> {
        ...
        db_connector.ActiveGameData.find(activeConnections[client.id],'socketRoom', (err, resultSet)=> {

            //if there are still active games with the disconnecting user as a player
            if(resultSet) {
                for(item of resultSet) {
                    db_connector.ActiveGameData.findByIdAndRemove(item._id,'socketRoom', (err, game)=> {
    ...
    client.on('end', (data) => {
        if(dg_interface.check_end(data) === true) {
            ...
            db_connector.ActiveGameData.findByIdAndRemove(data.ID_game,'socketRoom', (err, game)=> {
                // an error occured while getting the game
                if(err) {
                    client.emit('reject', dg_interface.emit_reject());
                } else {
